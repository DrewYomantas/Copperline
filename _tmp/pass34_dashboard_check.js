...
function _panelRenderSessionChrome(rows, fi) {
  ...
  shortcutsEl.innerHTML = 'â†/â†’ move â€¢ A approve â€¢ Shift+A approve+next â€¢ S schedule â€¢ Shift+S schedule+next â€¢ N next';
}
...
function renderTable() {
  ...
  tbody.innerHTML = filteredRows.map((row, fi) => {
    ...
    return `<tr class="${sentClass} ${repliedClass} ${scheduledClass}" onclick="openPanel(${fi})">
      ...
      <td class="td-status">${statusBadge(row)}${row.send_after && !row.sent_at ? `<div style="font-size:10px;color:var(--muted);margin-top:2px">${_formatSendAfter(row.send_after)}</div>` : ''}</td>
      <td class="td-actions" onclick="event.stopPropagation()">
        <div class="row-acts">
          ${approveBtn}
          <button class="act act-edit" onclick="openPanel(${fi})">Edit</button>
          ${actionable && !row.send_after ? `<button class="act" style="background:rgba(245,166,35,.13);color:var(--amber);border:none" onclick="rowSchedule(${gi},${JSON.stringify(row.business_name)})" title="Schedule for tomorrow">ðŸ•</button>` : ''}
...
async function fillPanel(row, fi, totalRows) {
  ...
  if (schedEl) {
    if (row.send_after && !row.sent_at) {
      const gi_s = allRows.indexOf(row);
      schedEl.innerHTML =
        `<span class="sched-time">ðŸ• Scheduled: ${_formatSendAfter(row.send_after)}</span>` +
...
async function panelScheduleTomorrow() {
  ...
  try {
    ...
    toast('Scheduled for tomorrow', 'ok');
    return true;
  } catch(e) {
...
async function panelReschedule(days) {
  ...
  try {
    ...
    renderTable();
    fillPanel(row, panelIdx, _panelCurrentRows().length);
    toast('Rescheduled for ' + _formatSendAfter(row.send_after), 'ok');
    return true;
  } catch(e) {
    toast('Connection error', 'err');
  }
...
async function bulkSchedule(mode) {
  ...
  clearSelection();
  await loadStats();
  toast(isClear ? `Cleared ${ok} schedule(s)` : `Scheduled ${ok} row(s)`, ok > 0 ? 'ok' : 'err');
}
...
async function rowSchedule(gi, businessName) {
  ...
  try {
    ...
    if (res.ok) { row.send_after = res.send_after; renderTable(); await loadStats(); toast('Scheduled: ' + businessName, 'ok'); }
...
async function rowUnschedule(gi, businessName) {
  ...
  try {
    ...
    if (res.ok) { row.send_after = ''; renderTable(); await loadStats(); toast('Schedule cleared', 'ok'); }
